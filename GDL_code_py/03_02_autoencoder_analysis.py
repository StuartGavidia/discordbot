...
# Specify the digits you want to reconstruct
desired_digits = [5]

# Find the indices of those digits in the test set
indices = []
for digit in desired_digits:
    idx = np.where(y_test == digit)[0]
    indices.append(idx[0])  # Take the first occurrence for simplicity

# Select the images for these indices
...
for i in range(len(indices)):
    ...
    ax = fig.add_subplot(2, len(indices), i+len(indices)+1)
    ax.axis('off')
    ax.imshow(img, cmap='gray_r')
